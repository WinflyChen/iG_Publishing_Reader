...
--local profile  = require("bookprofile")
    ...
    local function arrangPos()
        ...
        local booknum = 0
        print("#booksProfile===",#booksProfile)
		for i=1, #booksProfile do
			-- print("books", booksProfile[i].bookcode, booksProfile[i].expired)

   

			-- 沒有逾期的書放到 books table

			--if booksProfile[i].expired == "false" then
            if i > 1 then
				local tmp ={}
                ...
                print("bookcode.....=",tmp["bookcode"] )
                print("title.....=",tmp["title"] )
                print("expired.....=",tmp["expired"] )
                print("publisher.....=",tmp["publisher"] )
                print("author.....=",tmp["author"] )
                print("year.....=",tmp["year"] )
                print("download.....=",tmp["download"] )
                print("expirytime.....=",tmp["expirytime"] )
				table.insert(books, tmp)
			--end
            end
		end

        --如果booknum == 0 ， 表示書架沒書, 直接要求輸入下載書碼

        if booknum == 0  then
            --print("num=",gInputGroup.numChildren)
    ...
    local function checkReturned(event)
        if ( event.isError ) then
            ...
            print("checkReturned _ response ==",event.response)
    ...
    function getbookparameter()
       ...
       -- local bookcode = booksProfile[checkNum].bookcode
        ...
        --     --第一筆忽略
        --     print("expiredtime==",tonumber(booksProfile[i].expirytime)/1000)
        --     local tttt= os.time()
        --     print("os.time===",tttt)
        --     if(tonumber(booksProfile[i].expirytime)/1000 < os.time() and i > 1) then

        --         booksProfile[i].expired = "true"
    ...
    local function Button_Read(bookcode, page)
        ...
        local button = widget.newButton
        {
            --width  = 0,
            --height = 0,
            x      = display.contentWidth*0.1+150,
            y      = tab_y*0.3-56,
    ...
    local function Button_Close()
        ...
        local button = widget.newButton
        {
            ...
            -- font        = native.systemFontBold,
            -- fontSize    = 30,
            --defaultFile = "image/ic_highlight_remove_white_48dp.png",
            --overFile    = "image/ic_highlight_remove_grey600_48dp.png",
    ...
    local function Button_Delete_Yes(del_bookcode,ty)
        ...
        --     -- strokeColor = { default={ 0.55, 0.55, 0.55, 1 }, over={ 0.35, 0.35, 0.35, 1 } },
        --     -- strokeWidth = 2                   
        -- }button_yeslocal button = widget.newButton
        local xpos = 0
        print("ty===",ty)
        -- if ty == 1 then
	...
	function scene:change_page()
		...
		for i = 1, _pageSize do
			local w = math.floor((i-1)%3)
			local h = math.floor((i-1)/3) 
			...
			-- if bookRect[i] ~= nil then 
			-- 	bookRect[i]:removeEventListener( "touch", covertouch )
			-- 	table.remove( bookRect[i] )
			-- 	bookRect[i] = nil 
			-- end

			local interval_W = (scrollView.width - _bkW*3) / 4
			if interval_W < 0 then interval_W = 10 end
			--print("iiiiiii", scrollView.width, _bkW*3, interval_W)

			local interval_H = (scrollView.height - _bkH*3) / 4
			if interval_H < 0 then interval_H = 10 end

			-- bookRect[i] = display.newRoundedRect( _bkW*0.5 + _bkW*(w-1)+_bkW+interval_W*(w+1), 
			-- 		                              _bkH*0.5 + _bkH*h, 
			-- 		                              _bkW , 
			-- 		                              _bkH-20, 
			-- 		                              1)
			...
			bookRect[i] = display.newRoundedRect( _bkW*0.5 + _bkW*(w-1)+_bkW+interval_W*(w+1), 
					                              _bkH*0.5 + _bkH*h + interval_H*(h+1), 
					                              _bkW , 
					                              _bkH-20, 
					                              1)

			bookRect[i].strokeWidth = 3
		...
		for i=1, _pageSize do
			...
			if books[i+add] ~= nil and bookRect[i] ~= nil then
		    	...
		    	if coverImage[i] ~= nil then
                   ...
                   -- print("expired=====",books[i+add].expired)
    ...
    --  刪除選項
    -----------------------------------------------------------
    local function deleteBook(bookNO,ty)
        Runtime:removeEventListener( "enterFrame", checkLongClickFn )
    	ScreenBlock()
    	Button_Delete_No()
    	Button_Delete_Yes(books[bookNO].bookcode,ty)
    ...
    local function ShowMessage( bookNO )
        ...
        if Show_Mata["title"].height > 650 then
        ...
        print("Show_Mata[title]=",Show_Mata["title"].text)
	    ...
	    Show_Mata["author"].anchorY = 0 Show_Mata["author"].anchorX = 0
         ...
         print("author]=",books[bookNO].author)
        ...
        print("publisher=",books[bookNO].publisher)
                ...
                if _moving == false  then
                    ...
                    if _inBlock == true then
                        if deletef == false then
                            pressTimer = os.time()
                           --print("expired===",books[_touchNO].expired)
  ...
  --       gNaviBar.icoShowOption(true)
	...
	function scene:show(event)
		...
		if event.phase == "will" then
            ...
            --backgroundImage = display.newImage(  "image/igpbg.png" ,system.ResourceDirectory,true  )
            --backgroundImage.x = display.contentWidth*0.5
            --backgroundImage.y = backgroundImage.height*0.5
			--scrollView:insert(backgroundImage)
         ...
         --    if gUrlText ~= nil then
	        --     gUrlText:toFront()
	        --     --gUrlText:setFillColor( 1, 1 ,0 )
	        --     gUrlText:setFillColor( math.random(), math.random(), math.random() )
	        -- end
